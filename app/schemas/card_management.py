...
class CCMCardBlockRequest(BaseModel):
    unblock_id: Optional[str] = Field(None, description="The unique ID returned in unblock_otp for verification")
    ...
    @field_validator("unblock_id")
    @classmethod
    def allow_empty_unblock_id(cls, v):
        if v == "" or v == "string":
            return None
        return v
