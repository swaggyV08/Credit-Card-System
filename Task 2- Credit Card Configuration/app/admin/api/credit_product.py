...
router = APIRouter(prefix="/admin/credit-products", tags=["Admin: Credit Products"])
...
def create_credit_product(
    ...
    """
    Creates a new pure financial Credit Product in DRAFT mode.
    """
    while True:
...
def get_all_credit_products(
    ...
    """Fetch all existing Credit Products with optional status filtering."""
...
def get_credit_product(
    ...
    """Fetch an existing Credit Product by ID."""
...
def update_credit_product_status(
    ...
    """
    Approves or Suspends a Credit Product based on the command query parameter.
    """
    if command == "approve":
