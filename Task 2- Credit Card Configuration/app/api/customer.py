...
from app.admin.schemas.card_issuance import CreditAccountResponse, CardResponse, CustomerCardResponse
...
from app.models.enums import KYCState,Country, PrimaryJurisdiction, DocumentCategory, KYCVerificationStatus
...
@router.post("/cif/kyc-documents")
def initiate_kyc_document(
...
@router.post("/cif/kyc-documents/{submission_id}")
...
@router.get("/credit-account", response_model=list[CreditAccountResponse])
def get_credit_account(
    user: User = Depends(get_current_authenticated_user),
    db: Session = Depends(get_db)
):
    """
    Fetch the user's active credit accounts.
    """
    profile = db.query(CustomerProfile).filter(CustomerProfile.user_id == user.id).first()
    if not profile:
        raise HTTPException(status_code=404, detail="Customer profile not found")
        
    accounts = db.query(CreditAccount).filter(CreditAccount.cif_id == profile.id).all()
    return accounts
...
def get_credit_cards(
    ...
    for card, account, product in cards_data:
        response.append({
            ...
            "credit_limit": float(account.sanctioned_limit),
            "available_limit": float(account.available_limit),
