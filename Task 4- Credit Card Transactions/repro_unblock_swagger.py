...
def run_test():
    ...
    test_user = User(id=uuid.uuid4(), email="test_unblock_login@test.com", phone_number="9999999998", country_code="+91", is_active=True)
    ...
    card_id = uuid.uuid4()
    test_card = CCMCreditCard(id=card_id, user_id=test_user.id, card_number="4111111111111112", card_network="VISA", card_variant="CLASSIC", expiry_date="12/25", cvv_hash="abc", status=CCMCardStatus.BLOCKED_USER, credit_account=test_account, is_contactless_enabled=True, is_international_enabled=True, is_online_enabled=True, is_atm_enabled=True, is_domestic_enabled=True)
    ...
    # 2. Generate OTP with Wrong Purpose (Simulating Swagger Default)
    resp = client.post(f"/auth/otp/{unblock_id}?command=generate", json={"purpose": "LOGIN"})
    print("Generate OTP (LOGIN):", resp.json())
    ...
    # 3. Verify OTP with Wrong Purpose
    resp = client.post(f"/auth/otp/{unblock_id}?command=verify", json={"purpose": "LOGIN", "otp": known_otp})
    print("Verify OTP (LOGIN):", resp.json())
    ...
    assert resp.json()["message"] == "Card Active Again"
    print("SUCCESS: Cross-verification successful despite LOGIN purpose.")
