...
class CreditAccountAdminService:
    ...
    def get_account(db: Session, account_id: UUID) -> CCMCreditAccount:
        ...
        account.card_count = 1 if account.card_id else 0
        account.cards = [account.card] if account.card else []
        return account
    ...
    def list_accounts(
        ...
        # Add card_count to each account object for the schema to pick up
        for acc in accounts:
            acc.card_count = 1 if acc.card_id else 0
