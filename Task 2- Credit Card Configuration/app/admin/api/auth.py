...
from app.schemas.admin import AdminCreate, AdminEmailLogin, AdminContactLogin, AdminResponse, TokenResponse, AdminCreationResponse
...
@router.post("/login/contact", response_model=TokenResponse)
def login_admin_contact(data: AdminContactLogin, db: Session = Depends(get_db)):
    admin = db.query(Admin).filter(
        Admin.country_code == data.contact.country_code,
        Admin.phone_number == data.contact.phone_number
    ).first()
    if not admin:
        raise HTTPException(status_code=401, detail="Invalid contact or passcode")
    
    if not verify_value(data.passcode, admin.passcode_hash):
        raise HTTPException(status_code=401, detail="Invalid contact or passcode")
...
def add_admin(
    ...
    hashed_passcode = hash_value(data.passcode)
